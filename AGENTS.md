...
2. **Send** — on submit, the full `msgs` history (`{role, content}[]`, kept in memory only, not persisted) is passed through `fit()`, then POSTed to `{base}/chat/completions` with `stream: true`, `max_tokens` (only when set), and Bearer auth.
...
- **Error path** — non-2xx or network failures show a `.msg.error` bubble with the server's error body (truncated to 400 chars); the user message stays in `msgs` so a retry re-sends it, and the failed assistant bubble is removed.
...
- **Error handling** — one `try/catch` around the request in the submit handler; surface human-readable text in an error bubble rather than throwing.
...
- **Mocking an endpoint** — to test offline, run any tiny server that serves `index.html` at `/` and returns SSE chunks at `POST /v1/chat/completions`:
  ...
  (A throwaway Node mock was used during development and deleted; recreate ad hoc when needed.)
